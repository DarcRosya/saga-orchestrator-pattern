...
async def process_logistic(ctx: dict[str, Any], order_id: uuid.UUID) -> None:
    ...
    try:
        ...
        async with session_factory() as session:
            ...
            if order.status == OrderGlobalStatus.CANCELLED:
                log.info("Saga was already failed by another worker. Just saving my status.")
            ...
            if logistics_status == SagaStepStatus.SUCCESS:
                ...
                order.status = OrderGlobalStatus.CANCELLED
